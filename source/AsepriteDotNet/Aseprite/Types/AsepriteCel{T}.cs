...
/// </summary>
public abstract class AsepriteCel<T> where T: IColor, new()
{
    /// <summary>
    /// Gets the layer that this cel exists on.
    /// </summary>
    public AsepriteLayer<T> Layer { get; }
    ...
    internal AsepriteCel(AsepriteCelProperties celProperties, AsepriteLayer<T> layer)
    {
        Layer = layer;
        Location = new Point(celProperties.X, celProperties.Y);
        Opacity = celProperties.Opacity;
        UserData = new AsepriteUserData<T>();
    }
}
