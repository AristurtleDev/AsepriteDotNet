...
public static class AsepriteFrameExtensions
{
    ...
    //-----------------------------------------------------------------------------------------------------------------
    //
    //  Vectorized methods here
    //
    //-----------------------------------------------------------------------------------------------------------------
    public static Rgba32[] FlattenFrameVectorized(this AsepriteFrame frame, bool onlyVisibleLayers = true, bool includeBackgroundLayer = false, bool includeTilemapCels = true)
    {
        ArgumentNullException.ThrowIfNull(frame);
        ...
        for (int celNum = 0; celNum < cels.Length; celNum++)
        {
            ...
            if (cel is AsepriteImageCel imageCel)
            {
                BlendCelVectorized(result, imageCel.Pixels, imageCel.Layer.BlendMode, new Rectangle(imageCel.Location, imageCel.Size), frame.Size.Width, imageCel.Opacity, imageCel.Layer.Opacity);
            }
            else if (includeTilemapCels && cel is AsepriteTilemapCel tilemapCel)
            {
                BlendTilemapCelVectorized(result, tilemapCel, frame.Size.Width);
            }
        }
    ...
    private static void BlendCelVectorized(Span<Rgba32> backdrop, ReadOnlySpan<Rgba32> source, AsepriteBlendMode blendMode, Rectangle bounds, int frameWidth, int celOpacity, int layerOpacity)
    {
        byte opacity = Calc.MultiplyUnsigned8Bit(celOpacity, layerOpacity);
        ...
        //-------------------------------------------------------------------------------------------------------------
        //
        //  This is the original method below.  This processes cel pixel by pixel blending from backdrop (bottom) to
        //  the layer above (source).  I'm guess for simd/vectorization you would implement some method of passing
        //  a Vector4[] here to the AserpiteColorUtilities.Blend method?
        //
        //-------------------------------------------------------------------------------------------------------------
        /*
        for (int y = startY; y < endY; y++)
    ...
    private static void BlendTilemapCelVectorized(Span<Rgba32> backdrop, AsepriteTilemapCel cel, int frameWidth)
    {
        byte opacity = Calc.MultiplyUnsigned8Bit(cel.Opacity, cel.Layer.Opacity);
        ...
        BlendCelVectorized(backdrop, pixels, cel.Layer.BlendMode, bounds, frameWidth, cel.Opacity, cel.Layer.Opacity);
    }
