...
/// </summary>
public sealed class AsepriteFrame<TColor> where TColor : struct, IColor<TColor>
{
    private readonly List<AsepriteCel<TColor>> _cels;
    ...
    /// </summary>
    public ReadOnlySpan<AsepriteCel<TColor>> Cels => CollectionsMarshal.AsSpan(_cels);

    /// <summary>
    /// Gets the name of this frame.
    /// </summary>
    /// <remarks>
    /// Frames do not natively have a name in Aseprite.  A name is auto generated for each <see cref="AsepriteFrame{TColor}"/>
    ...
    internal AsepriteFrame(string name, int width, int height, int duration, List<AsepriteCel<TColor>> cels)
    {
        Name = name;
    ...
    internal void AddCel(AsepriteCel<TColor> cel) => _cels.Add(cel);
}
