...
/// </summary>
public sealed class AsepriteFrame<T> where T : IColor, new()
{
    private readonly List<AsepriteCel<T>> _cels;
    ...
    /// </summary>
    public ReadOnlySpan<AsepriteCel<T>> Cels => CollectionsMarshal.AsSpan(_cels);

    /// <summary>
    /// Gets the name of this frame.
    /// </summary>
    /// <remarks>
    /// Frames do not natively have a name in Aseprite.  A name is auto generated for each <see cref="AsepriteFrame{T}"/>
    ...
    internal AsepriteFrame(string name, int width, int height, int duration, List<AsepriteCel<T>> cels)
    {
        Name = name;
    ...
    internal void AddCel(AsepriteCel<T> cel) => _cels.Add(cel);
}
