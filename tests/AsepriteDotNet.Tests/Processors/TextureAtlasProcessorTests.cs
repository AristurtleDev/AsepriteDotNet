...
public sealed class TextureAtlasProcessorTestFixture
{
    ...
    public TextureAtlasProcessorTestFixture()
    {
        ...
        {
            new AsepriteImageLayer<SystemColor>(new AsepriteLayerProperties() {Flags = 1, BlendMode = 0, Opacity = 255 }, "layer")
        };
        ...
        {
            new AsepriteImageCel<SystemColor>(celProperties, layers[0], imageCelProperties, new SystemColor[] {Red, Red, Red, Red })
        };

        List<AsepriteCel<SystemColor>> frame1Cels = new List<AsepriteCel<SystemColor>>()
        {
            new AsepriteImageCel<SystemColor>(celProperties, layers[0], imageCelProperties, new SystemColor[] { Green, Green, Green, Green })
        };

        List<AsepriteCel<SystemColor>> frame2Cels = new List<AsepriteCel<SystemColor>>()
        {
            new AsepriteImageCel<SystemColor>(celProperties, layers[0], imageCelProperties, new SystemColor[] { Blue, Blue, Blue, Blue })
        };
        ...
        AsepriteFile = new AsepriteFile<SystemColor>(Name, palette, width, height, AsepriteColorDepth.RGBA, frames, layers, [], [], [], new AsepriteUserData<SystemColor>(), []);
    }
}
...
public sealed class TextureAtlasProcessorTests : IClassFixture<TextureAtlasProcessorTestFixture>
{
    ...
    private readonly SystemColor _ = new SystemColor(0, 0, 0, 0);     //  Represents a transparent pixel from padding/spacing, not source
    private readonly SystemColor r = new SystemColor(255, 0, 0, 255); //  Represents a red pixel
    private readonly SystemColor g = new SystemColor(0, 255, 0, 255); //  Represents a green pixel
    private readonly SystemColor b = new SystemColor(0, 0, 255, 255); //  Represents a blue pixel
    private readonly SystemColor t = new SystemColor(0, 0, 0, 0);     //  Represents a transparent pixel from source, not padding/spacing.
    ...
    {
        TextureAtlas<SystemColor> atlas = TextureAtlasProcessor.Process(_fixture.AsepriteFile);
    ...
    public void Process_Duplicate_Frame_Is_Merged()
    {
        ...
        TextureAtlas<SystemColor> atlas = TextureAtlasProcessor.Process(_fixture.AsepriteFile, options);
        ...
        SystemColor[] actual = atlas.Texture.Pixels.ToArray();
